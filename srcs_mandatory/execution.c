...
static void	ft_dda(t_game_instance *game)
{
	while (game->dda.hit == 0)
	{
		...
		if (game->dda.x < 0 || game->dda.y < 0
			...
			game->dda.hit = 1;
		else if (game->map_data.map.grid[(int)game->dda.y][(int)game->dda.x]
			== WALL)
			game->dda.hit = 1;
	}
}
...
static void	ft_cast_rays(t_game_instance *game)
{
	...
	while (x <= (WIN_WIDTH - 1))
	{
		...
		game->dda.ray_x_dir = game->map_data.player.dir_x
			+ game->map_data.player.plane_x * camera_x;
		game->dda.ray_y_dir = game->map_data.player.dir_y
			+ game->map_data.player.plane_y * camera_x;
		if (game->dda.ray_x_dir == 0)
...
int	ft_render(t_game_instance *game)
{
	...
	mlx_put_image_to_window(game->mlx_ptr,
		game->win_ptr, game->screen.img, 0, 0);
...
{
	game->win_ptr
		= mlx_new_window(game->mlx_ptr, WIN_WIDTH, WIN_HEIGHT, "CUB3D");
	if (!game->win_ptr)
